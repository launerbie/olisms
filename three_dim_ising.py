...
class Ising(object):
    def __init__(self, rij=40, kolom=40, diepte=40, b_field=0.0, temperature=10, 
        ...
        self.grid = self.makegrid(rij, kolom, diepte, aligned=aligned)

        self.rij = rij
        self.kolom = kolom
        self.diepte = diepte
        self.shape = (rij, kolom, diepte)
    ...
    def choose_site(self):
        ...
        site_z = np.random.randint(0,self.diepte)
        
        return site_x, site_y, site_z

    def makegrid(self, x, y, z, aligned=False):
        """ 
        ...
        if aligned:
            ...
            grid = np.random.choice([-1, 1], size=x*y*z).reshape(x, y, z)
            return np.array(grid, dtype='int8')

    def neighbors_no_logic(self, site):
        i, j, k = site
        
        left = (j + self.kolom - 1) % (self.kolom)
        right = (j + 1) % (self.kolom)
        above = (i + self.rij - 1) % (self.rij)
        below = (i + 1) % (self.rij)

        front = (k + self.diepte - 1) % (self.diepte)
        back = (i + 1) % (self.diepte)
    ...
    def evolve_wolff(self, iterations):
        ...
        while i < iterations:
            ...
            nbrs = self.neighbors_no_logic(seed)
            ...
            while len(perimeter) != 0:
                ...
                if seed_spin == g[site_to_test]:
                    ...
                    if determinant <= bond_probability: 
                        ...
                        nbrs = self.neighbors_no_logic(site_to_test)
                        for nbr in nbrs:
...
def get_arguments():
    ...
    parser.add_argument('-z', default=40,type=int, help="number of depth_units") 
